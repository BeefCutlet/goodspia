...
public class ArtistController {
    ...
    public ResponseEntity<?> modify(@Parameter(name = "아티스트 번호", description = "정보를 수정할 아티스트의 번호")
                                    @PathVariable Long artistId,
                                    @Parameter(name = "아티스트 정보", description = "수정할 아티스트의 정보")
                                    @RequestPart @Valid ArtistUpdateRequest artist,
                                    @Parameter(name = "프로필 이미지", description = "수정할 아티스트의 프로필 이미지")
                                    @RequestPart(required = false) MultipartFile profile) {
        //아티스트의 프로필 이미지 저장(갱신)
        String profileImageName = ImageUpload.uploadImage(profile);
        artist.setProfileImage(profileImageName);
